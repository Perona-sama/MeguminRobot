...
from MeguminRobot import DRAGONS, dispatcher
...
from MeguminRobot.modules.helper_funcs.extraction import (
    extract_user,
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status == "administrator" or user_member.status == "creator":
        message.reply_text("Bagaimana saya dimaksudkan untuk mempromosikan seseorang yang sudah menjadi admin?")
        return
    ...
    # atur perm yang sama dengan bot - bot tidak bisa menetapkan perm yang lebih tinggi dari itu sendiri!!
    ...
    bot.sendMessage(
        chat.id,
        f"Berhasil dipromosikan <b>{user_member.user.first_name or user_id}</b>!",
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status == "creator":
        message.reply_text("Baka!!, Orang ini MENCIPTAKAN obrolan, bagaimana cara saya menurunkannya?")
        return
    ...
    try:
        ...
        bot.sendMessage(
            chat.id,
            f"Berhasil diturunkan <b>{user_member.user.first_name or user_id}</b>!",
            parse_mode=ParseMode.HTML,
        )
...
def set_title(update: Update, context: CallbackContext):
    ...
    if user_member.status == "creator":
        message.reply_text(
            "Baka!!, Orang ini MENCIPTAKAN obrolan, bagaimana saya bisa mengatur judul khusus untuknya?"
        )
        return
    ...
    try:
        ...
        message.reply_text("Saya tidak dapat menyetel judul khusus untuk admin yang tidak saya promosikan!")
        return

    bot.sendMessage(
        chat.id,
        f"Berhasil menyetel judul untuk <code>{user_member.user.first_name or user_id}</code> "
        f"untuk <code>{html.escape(title[:16])}</code>!",
        parse_mode=ParseMode.HTML,
    )
...
def adminlist(update, context):
    ...
    except BadRequest:  # jika pesan asli dihapus
        return
