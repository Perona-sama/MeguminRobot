...
from MeguminRobot.modules.helper_funcs.handlers import CMD_STARTERS, SpamChecker
from MeguminRobot.modules.helper_funcs.misc import is_module_loaded
...
# Jika modul akan dimuat, maka atur semua penangan magis
if is_module_loaded(FILENAME):
    ...
    from MeguminRobot.modules.sql import disable_sql as sql
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text(
                    f"Menonaktifkan penggunaan `{disable_cmd}`",
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "MeguminRobot.modules." + args[0].rsplit(".", 1)[0]
            ...
            if disabled_cmds:
                ...
                update.effective_message.reply_text(
                    f"Menonaktifkan penggunaan `{disabled_cmds_string}`",
                    parse_mode=ParseMode.MARKDOWN,
                )
            ...
            if failed_disabled_cmds:
                ...
                update.effective_message.reply_text(
                    f"Commands `{failed_disabled_cmds_string}` tidak dapat dinonaktifkan",
                    parse_mode=ParseMode.MARKDOWN,
                )
    ...
    def enable(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text(
                    f"Mengaktifkan penggunaan `{enable_cmd}`", parse_mode=ParseMode.MARKDOWN
                )
            else:
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "MeguminRobot.modules." + args[0].rsplit(".", 1)[0]
    ...
    def __stats__():
        return f"• {sql.num_disabled()} item cacat, di seberang {sql.num_chats()} chats."
    ...
    *Admins only:*
    • `/enable <nama cmd>`*:* aktifkan perintah itu
    • `/disable <nama cmd>`*:* nonaktifkan perintah itu
