...
# formatter waktu dari uniborg
def t(milliseconds: int) -> str:
    """Memasukkan waktu dalam milidetik, untuk mempercantik waktu,
    sebagai tali"""
...
def airing(update: Update, context: CallbackContext):
    ...
    if len(search_str) == 1:
        update.effective_message.reply_text(
            "Beri Tahu Nama Anime :) ( /airing <nama anime>)"
        )
        return
...
def manga(update: Update, context: CallbackContext):
    ...
    msg = ""
    if "errors" in json.keys():
        update.effective_message.reply_text("Manga tidak ditemukan")
        return
    if json:
        json = json["data"]["Media"]
...
def user(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.effective_message.reply_text("Username tidak ditemukan.")
        return
...
def site_search(update: Update, context: CallbackContext, site: str):
    ...
    try:
        ...
        message.reply_text("Berikan sesuatu untuk dicari")
        return
    ...
    if site == "kaizoku":
        ...
        if search_result:
            result = f"<b>Hasil pencarian</b> <code>{html.escape(search_query)}</code> <b>on</b> @KayoAnime: \n"
            ...
            more_results = False
            result = f"<b>Tidak ada hasil untuk</b> <code>{html.escape(search_query)}</code> <b>on</b> @KaizokuAnime"
        ...
        result = f"<b>Hasil pencarian</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>AnimeKayo</code>: \n"
        for entry in search_result:

            if entry.text.strip() == "Nothing Found":
                result = f"<b>Tidak ada hasil untuk</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>AnimeKayo</code>"
                more_results = False
                break
