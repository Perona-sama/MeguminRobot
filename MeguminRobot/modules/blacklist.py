...
import MeguminRobot.modules.sql.blacklist_sql as sql
from MeguminRobot import dispatcher, LOGGER
    ...
    for text in split_text:
        if filter_list == "Kata-kata dalam daftar hitam saat ini dalam <b>{}</b>:\n".format(
            ...
            send_message(
                ...
                "Tidak ada kata dalam daftar hitam <b>{}</b>!".format(html.escape(chat_name)),
...
def add_blacklist(update, context):
    ...
    if len(words) > 1:
        ...
        if len(to_blacklist) == 1:
            send_message(
                update.effective_message,
                "Menambahkan daftar hitam <code>{}</code> in chat: <b>{}</b>!".format(
    ...
    if len(words) > 1:
        ...
        if len(to_unblacklist) == 1:
            if successful:
                send_message(
                    update.effective_message,
                    "Dihapus <code>{}</code> dari daftar hitam masuk <b>{}</b>!".format(
...
def blacklist_mode(update, context):
    ...
    if args:
        if args[0].lower() in ["off", "nothing", "no"]:
            ...
            if len(args) == 1:
                teks = """Sepertinya Anda mencoba menyetel nilai waktu untuk daftar hitam tetapi Anda tidak menentukan waktu;  Coba, `/blacklistmode tban <nilai waktu>`.
...
Contoh nilai waktu: 4m = 4 menit, 3h = 3 jam, 6d = 6 hari, 5w = 5 minggu."""
            ...
            if len(args) == 1:
                teks = """Sepertinya Anda mencoba menyetel nilai waktu untuk daftar hitam tetapi Anda tidak menentukan waktu;  Coba, `/blacklistmode tmute <nilai waktu>`.
...
Contoh nilai waktu: 4m = 4 menit, 3h = 3 jam, 6d = 6 hari, 5w = 5 minggu."""
        ...
        if getmode == 0:
            ...
            settypeblacklist = "bisukan sementara untuk {}".format(getvalue)
        if conn:
        ...
        if re.search(pattern, to_match, flags=re.IGNORECASE):
            try:
                if getmode == 0:
                    ...
                    bot.sendMessage(
                        chat.id,
                        f"Muted {user.first_name} untuk menggunakan kata dalam Daftar Hitam: {trigger}!",
                    )
                    return
                elif getmode == 4:
                    message.delete()
                    ...
                    if res:
                        bot.sendMessage(
                            chat.id,
                            f"Kicked {user.first_name} untuk menggunakan kata dalam Daftar Hitam: {trigger}!",
                        )
                    return
                elif getmode == 5:
                    message.delete()
                    ...
                    bot.sendMessage(
                        chat.id,
                        f"Banned {user.first_name} untuk menggunakan kata dalam Daftar Hitam: {trigger}",
                    )
                    return
                elif getmode == 6:
                    message.delete()
                    ...
                    bot.sendMessage(
                        chat.id,
                        f"Banned {user.first_name} sampai '{value}' untuk menggunakan kata dalam Daftar Hitam: {trigger}!",
                    )
                    return
                elif getmode == 7:
                    message.delete()
                    ...
                    bot.sendMessage(
                        chat.id,
                        f"Muted {user.first_name} sampai '{value}' untuk menggunakan kata dalam Daftar Hitam: {trigger}!",
                    )
                    return
...
def __chat_settings__(chat_id, user_id):
    ...
    return "Ada {} kata-kata yang masuk daftar hitam.".format(blacklisted)


def __stats__():
    return "• {} pemicu daftar hitam, di seberang {} chats.".format(
