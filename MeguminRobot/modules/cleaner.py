...
from MeguminRobot import ALLOW_EXCL, CustomCommandHandler, dispatcher
...
from MeguminRobot.modules.sql import cleaner_sql as sql
...
def set_blue_text_must_click(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if val in ("off", "no"):
            ...
            reply = "Pembersihan teks teks telah dinonaktifkan untuk <b>{}</b>".format(
            ...
            reply = "Pembersihan teks teks telah diaktifkan untuk <b>{}</b>".format(
                html.escape(chat.title)
            )
            message.reply_text(reply, parse_mode=ParseMode.HTML)
...
Pembersih teks biru menghapus semua perintah buatan yang dikirim orang-orang di obrolan Anda.
 • `/cleanblue <on/off/yes/no>`*:* perintah bersih setelah mengirim
 • `/ignoreblue <word>`*:* mencegah pembersihan otomatis perintah
 ...
 *Berikut adalah perintah hanya untuk Bencana, admin tidak dapat menggunakan ini:*
 • `/gignoreblue <word>`*:* secara global mengabaikan pembersihan teks blueteks dari kata yang disimpan di seluruh Megumin.
